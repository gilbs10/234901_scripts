...
def get_submission_results(sub_id, login_data):
    results_template = 'https://open.kattis.com/submissions/%d'
    site = requests.post(results_template % sub_id, cookies=login_data.cookies)
    return site
...
Overrides default guess (based on suffix of first filename)''')
    ...
    parser.add_argument('-f', '--force',
                   ...
                   action='store_true')
    parser.add_argument('-b', '--no_browser',
                   help='Don\'t ask to open problem in browser',
                   action='store_true')
...
extension "%s"''' % (ext,))
    ...
    try:
        open_submission(plain_result, cfg, args)
    except configparser.NoOptionError:
        pass
